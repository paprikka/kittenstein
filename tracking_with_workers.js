function workerTemplate(){
  ...
  onmessage = function(e) {
    ...
    const {pixels, width, height, colors, minGroupSize, maxDimension, minDimension} = message.data;
    if(message.type !== 'process') return;

    let results = [];
    colors.forEach(function(color) {
      results = results.concat(
        trackColor_(pixels, width, height, color, minGroupSize, maxDimension, minDimension)
      );
    });
  ...
  const colors = {};
  function registerColor(color, fn) {
    colors[color] = fn;
  }

  function trackPromise(pixels, width, height, color){
    return new Promise( function(resolve, reject) {
      resolve(trackColor_(pixels, width, height, color));
    });
  }
  // Default colors
  //===================

  registerColor('cyan', function(r, g, b) {
  ...
  registerColor('magenta', function(r, g, b) {
    var threshold = 50,
      dx = r - 255,
      dy = g - 0,
      dz = b - 255;
  ...
  registerColor('yellow', function(r, g, b) {
    var threshold = 50,
      dx = r - 255,
      dy = g - 255,
      dz = b - 0;
...
function trackColor_(pixels, width, height, color, minGroupSize, maxDimension, minDimension) {
    ...
    var neighboursW = getNeighboursForWidth_(width);
    ...
    for (var i = 0; i < height; i++) {
      for (var j = 0; j < width; j++) {
        ...
        if (currGroupSize >= minGroupSize) {
          var data = calculateDimensions_(currGroup, currGroupSize);
          if (data) {
            data.color = color;
            results.push(data);
          }
    ...
    const result = mergeRectangles_(results, maxDimension, minDimension);
    return result;
  };
   ...
   * @private
   */
  function getNeighboursForWidth_(width) {
    if (neighbours_[width]) {
      return neighbours_[width];
    }
   ...
   * @private
   */
  function calculateDimensions_(cloud, total) {
    var maxx = -1;
    var maxy = -1;
    var minx = Infinity;
    var miny = Infinity;
...
function mergeRectangles_(rects, maxDimension, minDimension) {
    var intersects;
    var results = [];
  ...
  console.log('worker template initialised');
};
...
(function() {
  ...
  const workerCode = workerTemplate
    .toString()
    .replace('function workerTemplate(){', '')
    .replace(/}$/, '');
  ...
  function trackPromise(ctx, pixels, width, height, color){
    return new Promise( function(resolve, reject) {
      resolve(ctx.trackColor_(pixels, width, height, color));
    });
  }
  ...
  tracking.ColorTracker.prototype.track = function(pixels, width, height) {
    ...
    if(!worker.onmessage) {
      worker.onmessage = (event) => this.emit('track', {
    ...
    worker.postMessage({
      type: 'process',
      data: {
        pixels, width, height, colors, minGroupSize, maxDimension, minDimension
      }
    });
    // var results = [];
    ...
    //   self.emit('track', {
    //     data: results.reduce( (acc, val)=> acc.concat(val), [] )
    //   });
    // });



    // (new Promise(function(resolve, reject){resolve(results)})).then(
    //   () => this.emit('track', {data: results})
    // )
